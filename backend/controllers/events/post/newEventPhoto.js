...
const newEventPhoto = async (req, res, next) => {
  ...
  try {
    ...
    const [photos] = await connection.query('SELECT id FROM events_photos WHERE id_event=?;', [idEvent]);
    ...
    if (req.files && req.files.photo) {
      ...
      await connection.query('INSERT INTO events_photos (photo, id_event) VALUES (?, ?);', [savedPhoto, idEvent]);
    }
