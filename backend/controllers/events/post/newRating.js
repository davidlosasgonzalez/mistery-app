...
const newRating = async (req, res, next) => {
	...
	try {
		...
		const [
			...
			'SELECT id FROM ratings WHERE id_user=? AND id_event=?;',
			[idUser, idEvent]
		);
		...
		const [
			owner,
		] = await connection.query('SELECT id FROM events WHERE id_user=?;', [
			idUser,
		]);
		...
		const [
			...
			'INSERT INTO ratings (score, id_user, id_event) VALUES (?, ?, ?);',
			[score, idUser, idEvent]
		);
