...
const newUser = async (req, res, next) => {
  ...
  try {
    ...
    const [existingName] = await connection.query('SELECT id FROM users WHERE name=?;', [name]);
    ...
    await connection.query('INSERT INTO users (name, email, password, reg_code) VALUES (?, ?, SHA2(?, 512), ?);', [
      name,
      email,
      password,
      regCode,
